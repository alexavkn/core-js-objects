...
function removeProperties(obj, keys) {
  ...
  if (Array.isArray(keys)) {
    keys.map((element) => keysCopy.push(element));
  } else {
    keysCopy.push(keys);
  }
...
function makeWord(lettersObject) {
  const arrValues = Object.values(lettersObject);
  const arrLetter = arrValues.flat();

  Object.entries(lettersObject).forEach(([key, value]) => {
    value.forEach((element) => {
      arrLetter[element] = key;
    });
  });
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    ...
    if (cashRegister[25] < 0 || cashRegister[50] < 0) {
      return false;
    }
  }
  return true;
...
function group(array, keySelector, valueSelector) {
  ...
  array.forEach((element) => {
    map.set(
      keySelector(element),
      (map.get(keySelector(element)) === undefined
        ? []
        : map.get(keySelector(element))
      ).concat([valueSelector(element)])
    );
  });
  return map;
}
